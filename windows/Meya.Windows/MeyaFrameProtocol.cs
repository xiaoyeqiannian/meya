...
internal enum MeyaFrameType : byte
{
    Control = 1,
    AudioPcm16 = 2,
    Event = 3,
...
internal sealed record MeyaFrame(
    MeyaFrameType Type,
    byte[] Payload,
    ushort Flags = 0,
...
internal static class MeyaFrameCodec
{
    private static readonly byte[] Magic = "MEYA"u8.ToArray();
    internal const byte Version = 2;
    internal const int HeaderSize = 36;
    internal const int MaximumPayload = 16 * 1024 * 1024;

    internal static byte[] Encode(MeyaFrame frame)
    {
    ...
    internal static MeyaFrame Json(
        MeyaFrameType type,
        object value,
        Guid session = default,
        ulong sequence = 0)
    ...
    internal static JsonDocument ParseJson(MeyaFrame frame)
    {
    ...
    internal static byte[] GuidToNetworkBytes(Guid value) =>
        Convert.FromHexString(value.ToString("N"));

    internal static Guid GuidFromNetworkBytes(ReadOnlySpan<byte> value)
    {
        if (value.Length != 16)
        {
...
internal sealed class MeyaFrameDecoder
{
    private readonly List<byte> _buffer = [];
    internal long DiscardedBytes { get; private set; }

    internal IReadOnlyList<MeyaFrame> Feed(ReadOnlySpan<byte> data)
    {
        if (!data.IsEmpty)
        {
