...
- 自动反馈只在本机 SQLite 中持久化局部替换证据、确认/命中次数和激活结果，不保存完整识别文本、音频路径或应用名；
- 用户主动执行「学习刚才的修改」时，麦芽会在本机保存对应录音和整句修改事件；该数据不保存应用名，也不会上传；
...
- “管理个人词库… → 自动学习规则…”可查看每条规则的触发证据、确认次数和实际命中次数，并可单条撤销；
...
每次主动点击「学习刚才的修改」，系统会同时更新可立即生效的词汇规则，
并保存未来可用于微调的整句学习事件：
...
当前识别后端支持 MLX Whisper、MLX Qwen3-ASR，以及 FunASR Paraformer 本地模型。三种架构都使用同一套实时/最终模型选择和 IPC v2 帧协议；控制与事件使用有边界的 UTF-8 JSON 帧，音频使用原始 PCM16 帧。Paraformer Streaming 会建立持续会话，从 macOS 录音回调直接推送 480 ms PCM16 音频块并保留编码/解码缓存，不经过 Base64、定时轮询或临时 WAV 文件；非流式模型仍自动使用滚动窗口路径。常用模型：
...
若录音期间前台窗口变化，结果只复制到剪贴板，避免写入错误窗口。若 Windows 没有可用的录音输入端点，浮层会显示“未检测到麦克风”提示约 4 秒，而不是立即消失。
