...
- 反馈学习只持久化局部替换候选、确认次数和激活结果，不保存完整识别文本、音频路径或应用名；
...
通过当前模型 tokenizer 检测的形式才会启用。对 `NovaKit` 这类词典外专名不会猜测读音。
...
先把正确的标准术语加入个人词库。完成一次语音输入后：

- 如果修改后的文字仍在输入框中，菜单“学习刚才的修改”会优先自动读取差异；
- 如果内容已经发送，或者当前网页/应用不允许读取输入框，麦芽会显示上一句并要求输入或粘贴正确文本；
- 系统会明确展示实际学到的映射，例如 `诺瓦 → NovaKit`；没有命中现有标准术语时不会创建不受约束的替换规则。
