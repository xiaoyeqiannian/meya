...
public sealed record SessionTransition(SessionState State, params string[] Actions);
...
public static class SessionStateMachine
{
    ...
    public static SessionTransition Transition(SessionState state, SessionEvent @event) =>
        Transitions.TryGetValue((state, @event), out SessionTransition? transition)
            ? transition
            : new SessionTransition(state);

    public static string WireName(SessionState state) => state switch
    {
    ...
    public static SessionEvent ParseEvent(string value) => value switch
    {
